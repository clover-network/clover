...
use clover_traits::TransactionPaymentOps;
...
pub trait Trait: frame_system::Trait {
  ...
  type OnTransactionPayment2: TransactionPaymentOps<Self::AccountId, BalanceOf<Self>, PositiveImbalanceOf<Self>, NegativeImbalanceOf<Self>>;
}
  ...
  BalanceOf<T>: FixedPointOperand
{
    ...
    T::Call: Dispatchable<Info=DispatchInfo>,
  {
    ...
    let (fee_deductable, fee_must_pay) = Self::compute_fee(len, &dispatch_info, 0u32.into());
    let partial_fee = fee_deductable.saturating_add(fee_must_pay);
...
BalanceOf<T>: Send + Sync + FixedPointOperand,
{
  ...
  fn withdraw_fee(
    ...
    info: &DispatchInfoOf<T::Call>,
    len: usize,
  ) -> Result<(BalanceOf<T>, BalanceOf<T>, NegativeImbalanceOf<T>, NegativeImbalanceOf<T>), TransactionValidityError> {
    let tip = self.0;
    let (tx_fee, tip) = Module::<T>::compute_fee(len as u32, info, tip);

    match T::OnTransactionPayment2::payfee(who, tx_fee, tip) {
      Ok((deductable, charged)) => Ok((tx_fee, tip, deductable, charged)),
  ...
  T::Call: Dispatchable<Info=DispatchInfo, PostInfo=PostDispatchInfo>,
{
  ...
  /// Data from pre_dispatch
  /// deductable fee paied, account balance charged
  type Pre = (BalanceOf<T>, Self::AccountId,
              Option<NegativeImbalanceOf<T>>, Option<NegativeImbalanceOf<T>>,
              BalanceOf<T>, BalanceOf<T>);
  ...
  fn validate(
    ...
    let (tx_fee, tip, _, _) = self.withdraw_fee(who, info, len)?;
    let fee = tx_fee.saturating_add(tip);
    Ok(ValidTransaction {
  ...
  fn pre_dispatch(
    ...
    let (tx_fee, tip, imbalance_deductable, imbalance_charged) = self.withdraw_fee(who, info, len)?;
    Ok((self.0, who.clone(), Some(imbalance_deductable), Some(imbalance_charged), tx_fee, tip))
  }
  ...
  fn post_dispatch(
    ...
    // refund only happens when something are really charged...
    if let (tip, who, Some(imbalance_deductable), Some(imbalance_charged),
            tx_fee, tx_tip) = pre {
      let deducted_balance = imbalance_deductable.peek();
      let charged_balance = imbalance_charged.peek();
      if !deducted_balance.is_zero() ||  !charged_balance.is_zero() {
        let (post_tx_fee, post_tip) = Module::<T>::compute_actual_fee(
          len as u32,
          info,
          post_info,
          tip,
        ...
        // refund priority:
        // 1. actual charged funds
        // 2. deducted funds
        let (deductable_refund, charged_refund) = if post_tx_fee < deducted_balance {
          let deducted_refund = deducted_balance.saturating_sub(post_tx_fee);
          let charged_refund = charged_balance.saturating_sub(post_tip);
          (deducted_refund, charged_refund)
        } else {
          // post_tx_fee >= deducted_balance
          // Note: the deductable imbalance in pre might be less than
          // actual post_tx_fee, because the deductable amount might not be enough,
          // so the rest of deductable balance goes to the charged part
          let tx_fee_charged = post_tx_fee.saturating_sub(deducted_balance);
          let post_charge = post_tip.saturating_add(charged_balance);
          let charged_refund = charged_balance.saturating_sub(post_charge);
          (<BalanceOf<T>>::zero(), charged_refund)
        };

        let calc_actual = |im_left: NegativeImbalanceOf<T>, im_right: NegativeImbalanceOf<T>,
        refund_left: PositiveImbalanceOf<T>, refund_right: PositiveImbalanceOf<T>|
            -> Result<(NegativeImbalanceOf<T>, NegativeImbalanceOf<T>), PositiveImbalanceOf<T>>{
          let actual_left = im_left.offset(refund_left)?;
          let actual_right = im_right.offset(refund_right)?;
          Ok((actual_left, actual_right))
        };

        let actual_payment = match T::OnTransactionPayment2::on_payback(&who, deductable_refund, charged_refund) {
          Ok((refund_reduct_imbalane, refund_charged_imbalance)) => {
            // The refund cannot be larger than the up front payed max weight.
            // `PostDispatchInfo::calc_unspent` guards against such a case.
            match calc_actual(imbalance_deductable, imbalance_charged,
                              refund_reduct_imbalane, refund_charged_imbalance) {
          ...
          Err(_) => (imbalance_deductable, imbalance_charged),
        };
